...
+ (instancetype)metaWithClassInfo:(YYClassInfo *)classInfo propertyInfo:(YYClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    //判断类是写了modelCustomClassForDictionary:这个方法
    ...
    //[类 instancesRespondToSelector]判断的是该类的实例是否包含某方法
...
- (instancetype)initWithClass:(Class)cls {
    ...
    /*
      _ivarInfos 得到类中的变量;
      _methodInfos  得到类中所有的方法;
      _propertyInfos 得到类中所有的属性;
     */
    ...
    if ([cls respondsToSelector:@selector(modelPropertyBlacklist)]) {
        //黑名单中的所有属性在模型转换过程将被忽略。返回nil忽略这个功能。
    ...
    // Get white list
    NSSet *whitelist = nil;
    //如果属性不在白名单,在模型转换过程将被忽略。返回nil忽略这个功能
    ...
    while (curClassInfo && curClassInfo.superCls != nil) { // recursive parse super class, but ignore root class (NSObject/NSProxy)
        // curClassInfo.propertyInfos.allValues; 遍历所有属性的个数
        for (YYClassPropertyInfo *propertyInfo in curClassInfo.propertyInfos.allValues) {
            ...
            //去除模型里面的对应的key到allPropertyMetas里面
...
+ (instancetype)metaWithClass:(Class)cls {
    ...
    NSDictionary *Dic = (__bridge NSDictionary *)(cache);
    ...
    NSDictionary *dic2 = (__bridge NSDictionary *)(cache);
    //放回元数据
    return meta;
}
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    //将model转化为我们看操作的YYModel
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:[model class]];
    if (!modelMeta || modelMeta->_keyMappedCount == 0) return nil;
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:64];
    //避免循环引用，在arc环境下为了兼容，用 __unsafe_unretained 替代 __weak 解决强引用循环的问题
    ...
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _YYModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        // 是否实现了自定义的映射中的转字典 modelCustomTransformToDictionary:
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    /*
     CFArrayApplyFunction CFDictionaryApplyFunction
     目的：遍历Array数组中的元素，没一个传入一个函数中进行的处理
     优点：便利容器类能带来的性能的提升
     函数处理的方式：
     */
     /*********************************
      三者的转化 model meta dic
      *********************************/
    ...
    //modelCustomTransformFromDictionary:当 JSON 转为 Model 完成后，该方法会被调用。
    // 你可以在这里对数据进行校验，如果校验不通过，可以返回 NO，则该 Model 会被忽略。
    // 你也可以在这里做一些自动转换不能完成的工作。
